// ==================================================
// STEP配信システム Code.gs Ver.30.3
// 安定化版：生徒キャッシュ・欠席直接更新・履歴代表本文
// ==================================================
const VERSION = 'Ver.30.3';
...
function sendSelected_(d){
  ...
  ids.forEach(id=>{
    ...
    const recipients=[r.mail1,r.mail2].filter(x=>String(x||'').trim()).map(x=>String(x).trim());
...
function getAbsences(){
  // Ver.30：画面更新・手動更新では元の「★欠席遅刻」シートを直接読みます。
  const list=readAbsencesDirect_();
  return list.map(({dateObj,...rest})=>rest);
}
