// ==================================================
// STEP配信システム Code.gs Ver.18
// GitHub側 Ver.18 とセットで使用
// ==================================================
const VERSION = 'Ver.18';
...
function setupStepMailSystem(){const ss=SpreadsheetApp.getActiveSpreadsheet();ensureSetting_(ss);ensureTemplate_(ss);ensureHistory_(ss);SpreadsheetApp.getUi().alert('STEP配信システム '+VERSION+' 初期設定完了');}
...
function doPost(e){try{const d=JSON.parse(e.postData.contents); let r; if(d.action==='sendSelected')r=sendSelected_(d); else if(d.action==='archiveHistory')r=archiveHistory_(d.id); else if(d.action==='saveTemplate')r=saveTemplate_(d,false); else if(d.action==='saveTemplateAs')r=saveTemplate_(d,true); else if(d.action==='deleteTemplate')r=deleteTemplate_(d.id); else throw new Error('不明なactionです'); return jsonOut_(r);}catch(err){return jsonOut_({error:true,message:err.message});}}
function normalizeGrade_(g){return String(g||'').replace(/[０-９]/g,s=>String.fromCharCode(s.charCodeAt(0)-65248)).replace(/　| /g,'').trim();}
function getStudentList(){const s=getSettings_(); const ss=SpreadsheetApp.openById(s['生徒マスタID']||DEFAULT_MASTER_ID); const sh=ss.getSheetByName(MASTER_SHEET_NAME); const v=sh.getDataRange().getValues(); const h=v[0]; const col={active:1,id:h.indexOf('生徒番号'),name:h.indexOf('生徒氏名'),school:h.indexOf('校舎'),grade:h.indexOf('学年'),mail1:h.indexOf('メールアドレス（保護者）'),mail2:h.indexOf('メールアドレス２')}; const out=[]; for(let i=1;i<v.length;i++){const r=v[i], flag=r[col.active]; if(!(flag===1||flag===0||flag==='1'||flag==='0')) continue; if(!r[col.id]||!r[col.name])continue; if(!r[col.mail1]&&!r[col.mail2])continue; let school=r[col.school]; if(school==='神領')school='神領校'; if(school==='大手')school='大手町校'; out.push({id:String(r[col.id]),name:String(r[col.name]),school:String(school||''),grade:normalizeGrade_(r[col.grade])});} return out;}
...
function sendSelected_(d){const s=getSettings_(); const ids=(d.studentIds||[]).map(String); if(!ids.length)throw new Error('送信先が選択されていません'); const ss=SpreadsheetApp.openById(s['生徒マスタID']||DEFAULT_MASTER_ID); const sh=ss.getSheetByName(MASTER_SHEET_NAME); const v=sh.getDataRange().getValues(), h=v[0]; const col={id:h.indexOf('生徒番号'),name:h.indexOf('生徒氏名'),school:h.indexOf('校舎'),grade:h.indexOf('学年'),mail1:h.indexOf('メールアドレス（保護者）'),mail2:h.indexOf('メールアドレス２')}; let sent=0, names=[], errors=[], attNames=[]; const attachments=(d.attachments||[]).map(a=>{attNames.push(a.name);return Utilities.newBlob(Utilities.base64Decode(a.data),a.type||MimeType.PLAIN_TEXT,a.name)}); ids.forEach(id=>{const r=v.find(x=>String(x[col.id])===id); if(!r)return; const name=r[col.name], school=r[col.school], grade=normalizeGrade_(r[col.grade]); const to=[r[col.mail1],r[col.mail2]].filter(Boolean).join(','); if(!to){errors.push(name+'：メールなし');return;} const body=String(d.body||'').replaceAll('{{生徒名}}',name).replaceAll('{{日付}}',d.dateText||'').replaceAll('{{曜日}}',d.weekday||'').replaceAll('{{時間帯}}',d.timeText||'').replaceAll('{{電話番号}}',phone_(school,s)); GmailApp.sendEmail(to,d.subject||'',body,{name:s['送信者名']||'個別指導STEP',attachments}); sent++; names.push(`${grade} ${name}さん`);}); saveHistory_(d,names,sent,attNames,errors); return {ok:true,sentCount:sent,sentNames:names,errors};}
function saveHistory_(d,names,sent,attNames,errors){ensureHistory_(SpreadsheetApp.getActiveSpreadsheet()); SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET_HISTORY).appendRow([Utilities.getUuid(),new Date(),Utilities.formatDate(new Date(),'Asia/Tokyo','yyyy/MM/dd'),d.templateId||'',d.subject||'',d.body||'',names.join('、'),sent,d.dateValue||'',d.weekday||'',d.timeText||'',attNames.join('、'),errors.length?errors.join('\n'):'OK',1]);}
...
function archiveHistory_(id){const sh=SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET_HISTORY); const v=sh.getDataRange().getValues(); for(let i=1;i<v.length;i++){if(String(v[i][0])===String(id)){sh.getRange(i+1,14).setValue(0);return{ok:true};}} return{ok:false};}
...
function getAbsences(){const s=getSettings_(); const ss=SpreadsheetApp.openById(s['欠席遅刻シートID']||DEFAULT_ABSENCE_ID); const sh=ss.getSheetByName(ABSENCE_SHEET_NAME); const v=sh.getDataRange().getValues(); const today0=new Date(); today0.setHours(0,0,0,0); const out=[]; for(let i=1;i<v.length;i++){const r=v[i], date=r[3]; if(!(date instanceof Date))continue; const d=new Date(date); d.setHours(0,0,0,0); if(d<today0)continue; out.push({dateLabel:dateLabel_(date),isToday:d.getTime()===today0.getTime(),school:r[2]||'',name:r[1]||'',reason:r[5]||'',kind:r[6]||'',other:r[7]||''});} out.sort((a,b)=>a.dateLabel.localeCompare(b.dateLabel)); return out;}
