...
function makeBody(){let b=$('bodyInput').value||''; const d=$('dateInput').value; const t=timeText(); const sample=[...selected.values()][0]?.name||'山田太郎'; return b.replaceAll('{{生徒名}}',sample).replaceAll('{{日付}}',jpDateOnly(d)).replaceAll('{{曜日}}',W[new Date(d+'T00:00:00').getDay()]).replaceAll('{{時間帯}}',t).replaceAll('{{電話番号}}','0568-41-8937')}
...
async function load(){
  ...
  const cached = localStorage.getItem('step_students_v30_1');
  if(cached){
    try{
  ...
  api.getStudents().then(list=>{
    students=list||[];
    localStorage.setItem('step_students_v30_1', JSON.stringify(students));
    renderStudents();
