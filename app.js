...
async function load(){
  ...
  // Ver.30：まずブラウザ保存の生徒一覧を即表示し、裏で最新を取得します。
  const cached = localStorage.getItem('step_students_v30_3');
  if(cached){
    try{
  ...
  const cachedTemplates = localStorage.getItem('step_templates_v30_3');
  if(cachedTemplates){
    try{
  ...
  try{
    ...
    localStorage.setItem('step_templates_v30_3', JSON.stringify(templates));
    $('templateSelect').innerHTML=templates.map(t=>`<option value="${t.id}">${t.name}</option>`).join('');
    applyTemplate();
  }catch(e){
    if(!templates.length){
  ...
  api.getStudents().then(list=>{
    students=list||[];
    localStorage.setItem('step_students_v30_3', JSON.stringify(students));
    renderStudents();
...
async function loadAbsences(){const data=await api.getAbsences();$('absenceList').innerHTML=data.map(a=>`<div class="absenceItem ${a.isToday?'today':''}"><b>${a.dateLabel}</b><div>${a.school}　${a.name}</div><div>${a.kind}　${a.reason||''}</div><div class="muted">${a.other||''}</div></div>`).join('')||'<div class="muted">本日以降の欠席遅刻連絡はありません。</div>'; const t=$('absenceAutoStatus'); if(t){t.textContent='最終確認：'+new Date().toLocaleTimeString('ja-JP',{hour:'2-digit',minute:'2-digit'});}}
