...
async function load(){
  ...
  const cached = localStorage.getItem('step_students_v30');
  if(cached){
    try{
  ...
  api.getStudents().then(list=>{
    students=list||[];
    localStorage.setItem('step_students_v30', JSON.stringify(students));
    renderStudents();
