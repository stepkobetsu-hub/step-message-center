...
async function load(){
  ...
  // Ver.31：まずブラウザ保存の生徒一覧を即表示し、裏で最新を取得します。
  ...
  const cachedTemplates = localStorage.getItem('step_templates_v31');
  if(cachedTemplates){
    try{
  ...
  try{
    ...
    localStorage.setItem('step_templates_v31', JSON.stringify(templates));
    $('templateSelect').innerHTML=templates.map(t=>`<option value="${t.id}">${t.name}</option>`).join('');
    applyTemplate();
  }catch(e){
    if(!templates.length){
  ...
  api.getStudents().then(list=>{
    students=list||[];
    localStorage.setItem('step_students_v31', JSON.stringify(students));
    renderStudents();
...
async function loadAbsences(){const cached=localStorage.getItem('step_absences_v31'); if(cached){try{renderAbsences(JSON.parse(cached)||[])}catch(e){}} const data=await api.getAbsences(); localStorage.setItem('step_absences_v31',JSON.stringify(data||[])); renderAbsences(data);}
