...
function bindEvents() {
  ...
  $('toggleBodyButton').addEventListener('click', toggleBodyEdit);
  $('closeBodyButton').addEventListener('click', closeBodyEdit);
...
function toggleBodyEdit() {
  const area = $('bodyEditArea');
  const isHidden = area.classList.contains('hidden');
  area.classList.toggle('hidden', !isHidden);
  $('toggleBodyButton').textContent = isHidden ? '本文を隠す' : '本文変更';
  if (isHidden) $('bodyInput').focus();
}

function closeBodyEdit() {
  $('bodyEditArea').classList.add('hidden');
  $('toggleBodyButton').textContent = '本文変更';
  updatePreview();
}
